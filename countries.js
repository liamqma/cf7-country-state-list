 ...
 *														*
 ...
 <script language="javascript">print_country("country","NAME OF COUNTRY CODE HERE");</script>
 *
 *  Author: Liam Ma <liam.q.ma@gmail.com>
 *
 *
 */
...
function printCountry(countryId, selectedCountryCode) {
    var selectObject = document.getElementById(countryId);
    for (var countryCode in countries) {

        // If key is not property of array
        if (!countries.hasOwnProperty(countryCode)) continue;

        // Add option to select object
        selectObject.add(new Option(countries[countryCode], countryCode));

        // Selected
        if (countryCode == selectedCountryCode) {
            selectObject.value = countryCode;
        }
...
function printState(stateId, countryCode) {

    var selectObject = document.getElementById(stateId);

    // Remove options from select object
    removeOptions(selectObject);

    var statesArray = states[countryCode].split("|");

    for (var key in statesArray) {

        // If key is not property of array
        if (!statesArray.hasOwnProperty(key)) continue;

        selectObject.add(new Option(statesArray[key], statesArray[key]));
